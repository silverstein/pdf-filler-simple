...
| Manifest metadata repack | Pass locally | Updated `manifest.json` and `manifest.mcpb.json` install metadata so the Claude Desktop extension card now foregrounds Sign mode, Signature/Date zones, URL-to-PDF workflows, local signatures, rendering, and allowed-directory controls. `mcpb pack` schema validation passed and the packed MCPB manifest contains display name `PDF Tools - View, Fill, Sign, Manage Pages, Extract`; rebuilt MCPB SHA256 is `5b51faca0a1fbb224b1be88ffb4005f7e24b9237d8750206ab65333bd07b214d`. This is a metadata-only repack after the functional Desktop validation above; reinstall before replacing/announcing the official asset if strict install-card validation is required. |
...
`5b51faca0a1fbb224b1be88ffb4005f7e24b9237d8750206ab65333bd07b214d` so the
