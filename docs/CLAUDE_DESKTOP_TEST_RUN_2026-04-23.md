...
| Manifest metadata repack | Pass locally | Updated `manifest.json` and `manifest.mcpb.json` install metadata so the Claude Desktop extension card now foregrounds Sign mode, Signature/Date zones, URL-to-PDF workflows, local signatures, rendering, and allowed-directory controls. `mcpb pack` schema validation passed and the packed MCPB manifest contains display name `PDF Tools - View, Fill, Sign, Fetch URLs, Manage Pages`; rebuilt MCPB SHA256 is `46589af7794174780bec7a036b4c00a0dde49f971824d9681a3e69b4c3c04576`. This is a metadata-only repack after the functional Desktop validation above; reinstall before replacing/announcing the official asset if strict install-card validation is required. |
...
`46589af7794174780bec7a036b4c00a0dde49f971824d9681a3e69b4c3c04576` so the
install card foregrounds Sign mode and URL-to-PDF workflows. A fresh extracted
