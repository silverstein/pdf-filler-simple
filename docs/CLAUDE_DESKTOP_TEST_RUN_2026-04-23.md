...
## Release Hardening Follow-up - 2026-04-24

| Item | Status | Evidence |
| --- | --- | --- |
| Model-readable zone coordinates | Fixed | `detect_signature_zones` now includes a compact visible coordinate table in the text result, not only `structuredContent`. The text explicitly lists `type/page/x/y/width/height/label/confidence/source` and tells agents to use `apply_text` for date zones and `apply_signature` for signature zones. Mirrored in `pdf-toolkit-mcp-share/server/index.js`. |
| Coordinate regression test | Pass | Added `test/detect-signature-zones-tool.test.js`; it calls the MCP tool over stdio and asserts visible text includes the polished W-9 page 1 signature `x=130.7 y=513.8 width=244.9 height=16.0` and date `x=410.2 y=513.8 width=110.0 height=16.0`. |
| Focused zone tests | Pass | `npm test -- test/zone-detection.test.js test/golden-set-placement.test.js test/detect-signature-zones-tool.test.js` passed `25` tests; same existing Vitest close-timeout warning appeared after pass. |
| Full local suite | Pass | `npm test` passed `19` files / `179` tests; same existing Vitest close-timeout warning appeared after pass. |
| Production UI build | Pass | `npm run build:ui` passed and produced `dist-ui/index.html` `3,449.56 kB` (`843.34 kB` gzip). |
| Dev viewer smoke | Pass | `npm run smoke:ui-dev` passed: viewer and MCP bridge responded. |
| Agent-browser sign smoke | Pass after daemon restart | First sandboxed run hit expected `~/.agent-browser` permission wall. First escalated run hit stale daemon `Resource temporarily unavailable`; after killing the stale daemon, `npm run smoke:ui-sign` passed and opened the sign modal. |
| Agent-browser inspect/preview harness | Harness flaky | `npm run smoke:ui-inspect` passed after replacing accessibility snapshots with DOM assertions. `npm run smoke:ui-preview-zone` remained intermittently flaky in `agent-browser` (`click`/`eval` timed out with `SIGTERM`) despite DOM assertions. Treat this as a harness reliability issue, not a product assertion, and prefer Playwright MCP for release evidence until the daemon path is stabilized. |
| Playwright MCP inspect-region | Pass | In a live dev viewer at `127.0.0.1:4180`, Playwright clicked `Sign`, armed `Inspect region`, dragged on the PDF, and verified the preview modal opened with `Copy coordinates`, `Done`, `Create`, and coordinates `Page 1 - x:250.0 y:200.0 w:110.0 h:70.0 pt`. |
| Playwright MCP preview-to-zone | Pass | From the same preview modal, Playwright clicked `Create signature zone` and verified the sign modal opened with title `Confirm signature`; the sign panel included `Custom signature zone`. |
| Playwright MCP draw-signature | Pass | Playwright opened `Draw signature`, drew on the canvas, filled `playwright-smoke-*` / `Playwright Smoke User`, saved, and verified the draw modal closed while Sign mode remained visible. |
| Playwright MCP rotated overlays | Pass | Created 90/180/270-degree W-9 fixtures and verified rotated signature overlay geometry: 90 deg `18 x 244.9` at `left=263.2/top=130.7`; 180 deg `244.9 x 18` at `left=236.4/top=263.2`; 270 deg `18 x 244.9` at `left=510.8/top=236.4`; each label had a non-identity rotation transform. |
| XFA viewer repro | Pass local and Desktop | Reproduced same-path `force_xfa=true` fill into `.test-tmp-xfa-viewer-repro/w9-xfa.pdf`; `pdfjs-dist` loaded the output as 4 pages and the dev viewer rendered it with `Fields (22)`. After final `0.8.6` reinstall, fresh Claude Desktop chat `PDF Tools XFA display regression check` ran `read_pdf_fields`, `fill_pdf force_xfa=true` to `/Users/silverbook/Downloads/pdf-toolkit-release-hardening-2026-04-24/w9-xfa-desktop-086.pdf`, then `display_pdf`; Claude reported the output rendered in the embedded viewer with no `Invalid PDF Structure` or render failure. |
| W-9 visual polish | Pass | User screenshot showed the corrected W-9 boxes were on the right row but still slightly high/floaty. Tuned captioned arrow-marker zones from `y=510.8 height=18` to `y=513.8 height=16` for both page 1 signature and date. Playwright Sign-mode visual check confirmed the polished overlay at signature `x=130.7 y=513.8 width=244.9 height=16` and date `x=410.2 y=513.8 width=110 height=16`. |
| Sandy Springs non-standard PDF | Pass | Downloaded `https://up.sandyspringsga.gov/sites/default/files/2024-06/New%20Business%20License%20Application.pdf` through the toolkit helper. It is an 8-page, non-fillable municipal packet (`0` AcroForm fields). Added it to the golden-set eval with seven hand-labeled regions. Detection found page 5 acknowledgement date/signature rows, page 5 home-business signature/date rows, page 7 applicant signature/date rows, and page 8 authorized-officer signature row; Playwright Sign-mode screenshots verified the overlays land on the visible lines. |
| Post-polish focused tests | Pass | `npm test -- test/zone-detection.test.js test/golden-set-placement.test.js test/detect-signature-zones-tool.test.js` passed `25` tests with the Sandy Springs fixture included; same existing Vitest close-timeout warning appeared after pass. |
| Post-polish full gates | Pass | `npm test` passed `19` files / `179` tests; `npm run build:ui` passed; `npm run smoke:ui-dev` passed. Same existing Vitest close-timeout warning appeared after pass. |
| Post-polish rotated overlays | Pass | Playwright rechecked rotated W-9 fixtures after the height/y polish: 90 deg `16 x 244.9` at `left=262.2/top=130.7`; 180 deg `244.9 x 16` at `left=236.4/top=262.2`; 270 deg `16 x 244.9` at `left=513.8/top=236.4`; each label had a non-identity rotation transform. |
| Post-polish artifacts | Pass | Rebuilt `pdf-toolkit-mcp.zip` and `pdf-toolkit-mcp.mcpb` after the W-9 polish and Sandy Springs fixture. First post-polish local artifact SHA256s: MCPB `4fd90a3bafdfd2c19a9b340133acd4e9b654f3471d6425c7d50b0d44fb1a1204`; ZIP `cb89319ebe1e66ba8ef80fe321cfa056d641a683283b713b383c7518b645cda1`. |
| Installed-code coordinate check | Pass | Directly imported the installed Claude extension from `~/Library/Application Support/Claude/Claude Extensions/local.mcpb.open-document-alliance.pdf-toolkit` and confirmed manifest `version: 0.8.6`; installed `detectSignatureZones` returns W-9 page 1 signature `x=130.7 y=513.8 width=244.9 height=16` and date `x=410.2 y=513.8 width=110 height=16`. |
| Claude Desktop final coordinate/Sandy rerun | Pass | Fresh Claude Desktop chat `PDF Tools 0.8.6 signature zone detection verification` used the installed extension. Sonnet reproduced the visible raw coordinate table for W-9 page 1 signature/date exactly (`y=513.8`, `height=16.0`), stated the coordinates are model-readable in text, displayed the W-9, downloaded Sandy Springs to `/Users/silverbook/Downloads/pdf-toolkit-release-hardening-2026-04-24/sandy-springs-new-business-license.pdf`, detected `7` zones across pages `5`, `7`, and `8`, and displayed the Sandy PDF. No PDFs were signed or mutated in this coordinate/display run. |
| Claude Desktop W-9 Sign-tab visual | Pass | In the live Claude Desktop W-9 embed from the final `0.8.6` chat, switched to `Sign` mode and scrolled the page. The page 1 `Sign here` and `Date` overlays now sit on the certification signing/date row and the side panel lists `4 zones` (`SIGNATURE p1`, `DATE p1`, `SIGNATURE p5`, `DATE p5`). |
| Adversarial review: MCPB local screenshots | Fixed | Reviewer found the first rebuilt MCPB included local smoke screenshots (`sandy-page*-sign.png`, `w9-polished-sign.png`) because `.gitignore` ignored `*-sign.png` but `.mcpbignore` did not. Added `*-sign.png` to `.mcpbignore`, rebuilt artifacts, and verified `unzip -l pdf-toolkit-mcp.mcpb | rg "sign\\.png|sandy-page|w9-polished|\\.playwright-mcp|\\.test-tmp"` returns no matches. New local artifact SHA256s: MCPB `c6e9e4df2882f4c2f25266f0f9f54c88e249303e94826dd411b5269090a5e152`; ZIP `ad61574c42d5c4446fdd05a216ab1abc33220abe668de3a118c1df612ae9ff55`. |
| Adversarial review: Sandy fixture durability | Fixed | Reviewer noted the Sandy Springs fixture was live-URL-only. Added pinned SHA256 `ea8d22947cd13888d516e572b8d290c3a84b14b1ef707e0c7b4023f3e12b8230`, marked it `required_for_release`, and taught the golden-set test to verify pinned bytes and fail if a release run tries to skip required URL fixtures with `OFFLINE=1`. `RELEASE_GATES=1 npm test -- test/golden-set-placement.test.js` passed. |
| Adversarial review: preview-zone harness | Known harness caveat | Reviewer confirmed `npm run smoke:ui-preview-zone` remains a flaky `agent-browser` harness, not a green package-script gate. Release evidence for preview-to-zone remains the Playwright MCP live-browser pass above; keep this as a follow-up harness hardening item rather than product behavior evidence. |
...
| Manifest metadata repack | Pass locally | Updated `manifest.json` and `manifest.mcpb.json` install metadata so the Claude Desktop extension card now foregrounds action-only positioning: Fill, Sign, Merge, Split, Extract, plus URL-to-PDF workflows, local signatures, rendering, and allowed-directory controls. `mcpb pack` schema validation passed and the packed MCPB manifest contains display name `PDF Tools - Fill, Sign, Merge, Split, Extract`; rebuilt MCPB SHA256 is `e754aee83ee7a3a4ecdfb6e7d54b6404a3a84c199552afdb3b1adac7ee7e8104`. This is a metadata-only repack after the functional Desktop validation above; reinstall before replacing/announcing the official asset if strict install-card validation is required. |
...
for inspect, preview-to-zone, draw-signature, and rotated overlays is green via
Playwright MCP. The W-9 overlay polish and Sandy Springs non-standard PDF check
...
chat. A later metadata-only manifest repack produced MCPB hash
`e754aee83ee7a3a4ecdfb6e7d54b6404a3a84c199552afdb3b1adac7ee7e8104` so the
install card foregrounds Fill, Sign, Merge, Split, Extract positioning plus
URL-to-PDF workflows. A fresh extracted
share-package stdio install also passed as a clean local-MCP/Cursor-path
validation without touching real Cursor config. The only remaining pre-release
confidence item, if desired, is reinstalling the final metadata-repacked MCPB in
Claude Desktop or testing on a true second machine; no repo-local blocker
remains.
