...
| Manifest metadata repack | Pass locally | Updated `manifest.json` and `manifest.mcpb.json` install metadata so the Claude Desktop extension card now foregrounds Sign mode, Signature/Date zones, URL-to-PDF workflows, local signatures, rendering, and allowed-directory controls. `mcpb pack` schema validation passed and the packed MCPB manifest contains display name `PDF Tools - View, Fill, Sign, Merge, Split, Extract`; rebuilt MCPB SHA256 is `0beb90f05e75c3da9ae54c949ba04cbb59ee4982a6ad44b1da1066f81701cd88`. This is a metadata-only repack after the functional Desktop validation above; reinstall before replacing/announcing the official asset if strict install-card validation is required. |
...
`0beb90f05e75c3da9ae54c949ba04cbb59ee4982a6ad44b1da1066f81701cd88` so the
