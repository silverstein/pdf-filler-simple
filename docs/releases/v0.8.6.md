...
- Added full signing workflows: create/list/load saved signatures, prepare signing packets, add signature fields, apply signatures, apply text, inspect regions, preview regions, and create typed signing zones from previews.
...
The previous public GitHub release was `v0.7.3`, so `v0.8.6` includes more than the final placement polish. The broader release adds a signing surface, region inspection/rendering, PDF URL fetching, safer local directory controls, active-document lifecycle hardening, and a much larger automated/manual validation suite.
