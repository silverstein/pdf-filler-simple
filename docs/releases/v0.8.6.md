...
PDF Tools performs file operations locally and does not upload PDFs to a separate PDF service. Content returned to Claude Desktop or another MCP host may be processed under that host or model provider's data terms. This release targets Claude Desktop and local MCP hosts; it does not add a remote connector for Claude Cowork / web-hosted Claude.
