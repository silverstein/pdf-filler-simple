...
import { campaignV4Internals } from "./eval/deep-malformed-macos-campaign-v4.js";
import {
  QPDF_BUDGET_COMMAND_CLAIM_BOUNDARY,
} from "./eval/qpdf-macos-budget-exec.js";
import {
  campaignComparisonV4Internals,
} from "./eval/compare-deep-malformed-macos-campaign-v4.js";
...
function response({
  ...
  const successfulReadFields = !isError && toolName === "read_pdf_fields";
  const successfulRotate = !isError && toolName === "rotate_pdf_pages";
  const fields = successfulReadFields
    ? ["/_meta/viewUUID"]
    : successfulRotate
      ? [
          "/_meta/last_mutation_at",
          "/structuredContent/last_mutation_at",
        ]
      : [];
  const valueClass = successfulReadFields
    ? "pdf-view-uuid:epoch-millis:base36-6:near-call"
    : "iso-8601:utc-millisecond:near-call";
  const terminalKey = successfulReadFields
    ? "viewUUID"
    : successfulRotate
      ? "last_mutation_at"
      : null;
  return {
    ...
    response: {
      call_wall_clock: {
        end_ms: 1_785_349_626_100,
        start_ms: 1_785_349_626_000,
      },
      canonical_bytes: 100,
      content_items: 1,
      is_error: isError,
      scanner: {
        ...
        pass: true,
        protocol: "pdf-tools.deep-malformed-response-leak-scan.v4",
        raw_internal_matches: 0,
        response_normalization: {
          equality_groups: successfulRotate
            ? [{
                equal: true,
                paths: fields,
...
function request(tool = "rotate_pdf_pages") {
  return {
    protocol: "pdf-tools.deep-malformed-row-request.v4",
    fixture: {
...
function greenErrorRecord(tool = "rotate_pdf_pages") {
  ...
  return {
    protocol: "pdf-tools.deep-malformed-row-result.v4",
    request: rowRequest,
    fixture,
    ...
    inventory_policy: campaignV4Internals.rederiveInventoryPolicy({
      request: rowRequest,
      baseline,
      immediate: baseline,
      final: baseline,
...
function assertions(record, outer = record.final_inventory) {
  return campaignV4Internals.validateRowRecord(
    record,
    record.request,
    {
...
function plan() {
  ...
  return {
    protocol: "pdf-tools.deep-malformed-macos-campaign-plan.v4",
    ...
    candidate: {
      ...
      runner: evidenceIdentity(
        "/private/tmp/pdf-tools-v2-test/candidate/test/helpers/"
          + "deep-malformed-row-runner-v4.js",
      ),
    ...
    matrix: campaignV4Internals.expectedMatrix(),
  };
}
...
test("v4 plan freezes one exact, duplicate-free 13 x 6 matrix", () => {
  const valid = plan();
  assert.doesNotThrow(() => campaignV4Internals.validatePlan(valid));
  ...
  omitted.matrix.pop();
  assert.throws(() => campaignV4Internals.validatePlan(omitted));
  const duplicate = structuredClone(valid);
  duplicate.matrix[77] = structuredClone(duplicate.matrix[0]);
  assert.throws(() => campaignV4Internals.validatePlan(duplicate));
  const unknown = structuredClone(valid);
  unknown.matrix[0].fixture = "later-attribution-fixture";
  assert.throws(() => campaignV4Internals.validatePlan(unknown));
  const priorProtocol = structuredClone(valid);
  priorProtocol.protocol = "pdf-tools.deep-malformed-macos-campaign-plan.v3";
  assert.throws(() => campaignV4Internals.validatePlan(priorProtocol));
  ...
  for (const owned of candidateOwnedPaths(valid)) {
    ...
    assert.throws(() => campaignV4Internals.validatePlan(relocated));
  }
});
...
test("corpus comparison is rederived instead of trusting its pass label", () => {
  ...
  assert.doesNotThrow(() =>
    campaignV4Internals.validateCorpusComparison(
      comparison,
      validPlan.candidate,
      validPlan.corpus,
    ));
...
test("green typed-error row is accepted only with independently matching final state", () => {
  ...
  assert.equal(
    campaignV4Internals.validateRowRecord(
      record,
      record.request,
      {
        fixture: record.request.fixture.name,
...
test("rich read-content error classification is accepted only for that tool", () => {
  ...
  record.baseline_pdfjs_canary = response({ toolName: "read_pdf_content" });
  record.same_server_pdfjs_canary = response({ toolName: "read_pdf_content" });
...
test("row cannot self-certify output-on-error or extra split output", () => {
  ...
  splitExtra.product = response({ toolName: "split_pdf" });
...
test("invalid successful mutator output fails independent validation contract", () => {
  const record = greenErrorRecord();
  record.product = response({ toolName: "rotate_pdf_pages" });
  const rotateFile = {
    path: "rotated.pdf",
    kind: "file",
    mode: 0o600,
  ...
  record.inventory_policy = campaignV4Internals.rederiveInventoryPolicy({
  ...
  assertGreen(assertions(record));
  assert.equal(
    campaignComparisonV4Internals.outputSemantics(record.output_validation)
...
test("malformed inventories, server replacement, timeout, kill, and leaks fail closed", () => {
  ...
  const normalizationTamper = greenErrorRecord();
  normalizationTamper.product.response.scanner.response_normalization
    .freshness_policy.allowed_skew_ms = 2_000;
  cases.push([normalizationTamper, "product_response"]);

  const priorRowProtocol = greenErrorRecord();
  priorRowProtocol.protocol = "pdf-tools.deep-malformed-row-result.v3";
  cases.push([priorRowProtocol, "protocol"]);
...
test("two-run logical comparison ignores only declared nondeterminism", () => {
  ...
  rowB.candidate_stdout.record.product.response.call_wall_clock = {
    end_ms: 1_785_349_627_100,
    start_ms: 1_785_349_627_000,
  };
  assert.deepEqual(
    campaignComparisonV4Internals.logicalRow(rowA),
    campaignComparisonV4Internals.logicalRow(rowB),
  );
  ...
  rowB.candidate_stdout.record.product.response.scanner
    .response_normalization.freshness_policy.allowed_skew_ms = 2_000;
  assert.notDeepEqual(
    campaignComparisonV4Internals.logicalRow(rowA),
    campaignComparisonV4Internals.logicalRow(rowB),
  );
  rowB.candidate_stdout.record.product.response.scanner
    .response_normalization.freshness_policy.allowed_skew_ms = 1_000;
  ...
  assert.deepEqual(
    campaignComparisonV4Internals.normalizedPlan(planA),
    campaignComparisonV4Internals.normalizedPlan(planB),
  );
});
...
test("comparator rejects malformed embedded plans and authority drift", () => {
  ...
  const receipt = {
    protocol: "pdf-tools.deep-malformed-macos-campaign-result.v4",
    plan: {
  ...
  assert.equal(
    campaignComparisonV4Internals.receiptAuthoritiesValid(
      receipt,
      "run-a",
      authority,
    ),
  ...
  priorReceipt.protocol =
    "pdf-tools.deep-malformed-macos-campaign-result.v3";
  assert.equal(
    campaignComparisonV4Internals.receiptAuthoritiesValid(
      priorReceipt,
      "run-a",
