...
const RESULT_PROTOCOL = "pdf-tools.deep-malformed-macos-campaign-result.v2";
const ROW_REQUEST_PROTOCOL = "pdf-tools.deep-malformed-row-request.v2";
const ROW_RESULT_PROTOCOL = "pdf-tools.deep-malformed-row-result.v2";
...
function validatePlan(plan) {
  ...
  || !exactKeys(plan.candidate, [
    ...
    || plan.candidate.controller.path !== path.join(
      plan.candidate.path,
      "test/eval/deep-malformed-macos-campaign-v2.js",
    )
    || plan.candidate.runner.path !== path.join(
      plan.candidate.path,
      "test/helpers/deep-malformed-row-runner-v2.js",
    )
    ...
    throw new Error("Campaign plan violates its exact v2 schema");
  }
}
...
function validScanner(scanner, workRoot) {
  return exactKeys(scanner, [
    ...
    && scanner.protocol === "pdf-tools.deep-malformed-response-leak-scan.v2"
...
function validQpdfObservation(value) {
  return exactKeys(value, [
    ...
    && exactKeys(value?.stdout, ["bytes", "sha256", "text"])
    && exactKeys(value?.stderr, ["bytes", "sha256"])
    && value?.outcome === "close"
    && value?.code === 0
    && value?.signal === null
    && value?.timed_out === false
    && value?.output_overflow === false
    && integer(
      value?.stdout?.bytes,
      0,
      EXPECTED_EVIDENCE_LIMITS.qpdf_output_bytes,
    )
    && integer(
      value?.stderr?.bytes,
      0,
    ...
    && typeof value?.stdout?.text === "string"
    && Buffer.byteLength(value.stdout.text) === value.stdout.bytes
    && value.stdout.sha256
      === sha256(Buffer.from(value.stdout.text, "utf8"))
    && value.stderr.bytes === 0
    && value.stderr.sha256 === sha256(Buffer.alloc(0))
    && SHA256.test(value?.stdout?.sha256 ?? "")
    && SHA256.test(value?.stderr?.sha256 ?? "");
}
...
function validSemanticFingerprint(value) {
  ...
  if (!exactKeys(value, [
    ...
    || !exactKeys(value.canonicalization.command, [
      "code",
      "outcome",
      "output_overflow",
      "signal",
    ...
    || value.canonicalization.command.outcome !== "close"
    || value.canonicalization.command.code !== 0
    || value.canonicalization.command.signal !== null
    || value.canonicalization.command.timed_out !== false
    || value.canonicalization.command.output_overflow !== false
    || !exactKeys(
      value.canonicalization.command.stdout,
      ["bytes", "sha256"],
    )
    || value.canonicalization.command.stdout.bytes !== 0
    || value.canonicalization.command.stdout.sha256
      !== sha256(Buffer.alloc(0))
    || !exactKeys(
      value.canonicalization.command.stderr,
      ["bytes", "sha256"],
    )
    || value.canonicalization.command.stderr.bytes !== 0
    || value.canonicalization.command.stderr.sha256
      !== sha256(Buffer.alloc(0))
    ...
    || value.command.outcome !== "close"
    || value.command.code !== 0
    || value.command.signal !== null
    || value.command.timed_out !== false
    || value.command.output_overflow !== false
    || !exactKeys(value.command.stdout, ["bytes", "sha256"])
    || !integer(
      value.command.stdout.bytes,
      1,
      EXPECTED_EVIDENCE_LIMITS.semantic_fingerprint_bytes,
    )
    || !SHA256.test(value.command.stdout.sha256)
    || !exactKeys(value.command.stderr, ["bytes", "sha256"])
    || value.command.stderr.bytes !== 0
    || value.command.stderr.sha256 !== sha256(Buffer.alloc(0))
  ...
  const outputValid = exactKeys(
    ...
    && (mutatorSuccess
      ? record.output_validation.required === true
        ...
        && validSemanticFingerprint(outputRecord.semantic_fingerprint)
...
async function main() {
  if (process.platform !== "darwin"
    ...
    throw new Error("Usage: deep-malformed-macos-campaign-v2.js /canonical/plan.json");
  }
  ...
  process.stdout.write(`${canonicalJson({
    protocol: "pdf-tools.deep-malformed-macos-campaign-receipt-written.v2",
    receipt,
...
export const campaignV2Internals = Object.freeze({
  expectedMatrix,
