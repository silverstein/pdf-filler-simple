...
const PLAN_PROTOCOL = "pdf-tools.deep-malformed-macos-campaign-plan.v4";
const RESULT_PROTOCOL = "pdf-tools.deep-malformed-macos-campaign-result.v4";
const ROW_REQUEST_PROTOCOL = "pdf-tools.deep-malformed-row-request.v4";
const ROW_RESULT_PROTOCOL = "pdf-tools.deep-malformed-row-result.v4";
...
function validatePlan(plan) {
  ...
  || !exactKeys(plan.candidate, [
    ...
    || plan.candidate.controller.path !== path.join(
      plan.candidate.path,
      "test/eval/deep-malformed-macos-campaign-v4.js",
    )
    || plan.candidate.runner.path !== path.join(
      plan.candidate.path,
      "test/helpers/deep-malformed-row-runner-v4.js",
    )
    ...
    throw new Error("Campaign plan violates its exact v4 schema");
  }
}
...
function expectedResponseNormalization(toolName, isError) {
  if (isError || !["read_pdf_fields", "rotate_pdf_pages"].includes(toolName)) {
    return {
      equalityGroups: [],
      fields: [],
      retainedKeys: [],
  ...
  if (toolName === "read_pdf_fields") {
    return {
      equalityGroups: [],
      fields: ["/_meta/viewUUID"],
      retainedKeys: ["viewUUID"],
      valueClasses: {
        "/_meta/viewUUID":
  ...
  return {
    ...
    fields,
    retainedKeys: ["last_mutation_at"],
    valueClasses: Object.fromEntries(
      fields.map(field => [
        field,
...
function validResponseNormalization(normalization, toolName, isError) {
  const expected = expectedResponseNormalization(toolName, isError);
  return exactKeys(normalization, [
    "equality_groups",
    "excluded_fields",
    ...
    && normalization.tool_name === toolName
    && canonicalJson(normalization.excluded_fields)
      === canonicalJson(expected.fields)
    && exactKeys(normalization.excluded_occurrences, expected.fields)
    && expected.fields.every(field =>
      normalization.excluded_occurrences[field] === 1)
    && canonicalJson(normalization.excluded_value_classes)
      === canonicalJson(expected.valueClasses)
    && canonicalJson(normalization.equality_groups)
      === canonicalJson(expected.equalityGroups)
    && exactKeys(normalization.freshness_policy, [
      "allowed_skew_ms",
      "basis",
      "checked_fields",
    ])
    && normalization.freshness_policy.allowed_skew_ms === 1_000
    && normalization.freshness_policy.basis
      === "captured-call-wall-clock"
    && canonicalJson(normalization.freshness_policy.checked_fields)
      === canonicalJson(expected.fields)
    && exactKeys(
      normalization.retained_nonvolatile_occurrences,
      expected.retainedKeys,
    )
    && expected.retainedKeys.every(key =>
      integer(
        normalization.retained_nonvolatile_occurrences[key],
        0,
        EXPECTED_EVIDENCE_LIMITS.response_nodes,
      ));
}

function validScanner(scanner, workRoot, toolName, isError) {
  return exactKeys(scanner, [
    ...
    && scanner.protocol === "pdf-tools.deep-malformed-response-leak-scan.v4"
    ...
    && validResponseNormalization(
      scanner.response_normalization,
      toolName,
      isError,
    )
...
function validResponseObservation(
  observation,
  workRoot,
  { requireSuccess = false, toolName = null } = {},
) {
  return exactKeys(observation, ["outcome", "response"])
    ...
    && exactKeys(observation.response.call_wall_clock, ["end_ms", "start_ms"])
    && integer(
      observation.response.call_wall_clock.start_ms,
      0,
      Number.MAX_SAFE_INTEGER,
    )
    && integer(
      observation.response.call_wall_clock.end_ms,
      observation.response.call_wall_clock.start_ms,
      Number.MAX_SAFE_INTEGER,
    )
    && integer(
    ...
    && validScanner(
      observation.response.scanner,
      workRoot,
      toolName,
      observation.response.is_error,
    )
...
function validateRowRecord(
  ...
  const baselineValid = validResponseObservation(
    ...
    workRoot,
    { requireSuccess: true, toolName: "get_pdf_info" },
  );
  ...
  const pdfjsValid = pdfjsExpected
      ? validResponseObservation(
        ...
        workRoot,
        { requireSuccess: true, toolName: "read_pdf_content" },
      ) && validResponseObservation(
  ...
  const productValid = validResponseObservation(
    record?.product,
    workRoot,
    { toolName: matrixRow.tool },
  );
...
async function main() {
  if (process.platform !== "darwin"
    ...
    throw new Error("Usage: deep-malformed-macos-campaign-v4.js /canonical/plan.json");
  }
  ...
  process.stdout.write(`${canonicalJson({
    protocol: "pdf-tools.deep-malformed-macos-campaign-receipt-written.v4",
    receipt,
...
export const campaignV4Internals = Object.freeze({
  expectedMatrix,
