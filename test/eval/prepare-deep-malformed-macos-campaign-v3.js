...
/**
 * Prepare one exact 13 × 6 v3 product-campaign plan from a passing,
...
async function main() {
  if (process.platform !== "darwin" || process.argv.length !== 12) {
    throw new Error(
      "Usage: prepare-deep-malformed-macos-campaign-v3.js "
  ...
  const candidate = {
    ...
    controller: (await stableFile(
      path.join(candidatePath, "test/eval/deep-malformed-macos-campaign-v3.js"),
      MAX_FIRST_PARTY_BYTES,
    )).identity,
    runner: (await stableFile(
      path.join(candidatePath, "test/helpers/deep-malformed-row-runner-v3.js"),
      MAX_FIRST_PARTY_BYTES,
    )).identity,
  ...
  process.stdout.write(`${canonicalJson({
    protocol: "pdf-tools.deep-malformed-macos-campaign-plan-prepared.v3",
    logical_run_label: logicalRunLabel,
