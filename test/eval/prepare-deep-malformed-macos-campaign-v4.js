...
/**
 * Prepare one exact 13 × 6 v4 product-campaign plan from a passing,
...
import { campaignV4Internals } from "./deep-malformed-macos-campaign-v4.js";
import {
...
async function main() {
  if (process.platform !== "darwin" || process.argv.length !== 12) {
    throw new Error(
      "Usage: prepare-deep-malformed-macos-campaign-v4.js "
      + "/candidate /supervisor /build-receipt /corpus-comparison "
      + "/qpdf /qpdf-budget-exec /qpdf-budget-build-receipt "
      + "/attempt-root /plan-output /logical-run-label",
    );
  }
  const [, , candidatePath, supervisorPath, buildReceiptPath,
    corpusComparisonPath, qpdfRequestedPath, qpdfBudgetExecPath,
    qpdfBudgetBuildReceiptPath, attemptRoot, planOutput, logicalRunLabel
  ] = process.argv;
  ...
  const candidate = {
    ...
    controller: (await stableFile(
      path.join(candidatePath, "test/eval/deep-malformed-macos-campaign-v4.js"),
      MAX_FIRST_PARTY_BYTES,
    )).identity,
    runner: (await stableFile(
      path.join(candidatePath, "test/helpers/deep-malformed-row-runner-v4.js"),
      MAX_FIRST_PARTY_BYTES,
    )).identity,
  ...
  const qpdfBudgetExec = {
    binary: (await stableFile(
      qpdfBudgetExecPath,
      MAX_FIRST_PARTY_BYTES,
    )).identity,
    build_receipt: (await stableFile(
  ...
  const qpdfBudgetReceipt = await canonicalJsonFile(
    qpdfBudgetBuildReceiptPath,
    MAX_BUILD_RECEIPT_BYTES,
  );
  if (qpdfBudgetReceipt.value?.protocol !== QPDF_BUDGET_BUILD_PROTOCOL
      || qpdfBudgetReceipt.value?.testing !== false
      || canonicalJson(qpdfBudgetReceipt.value?.binary)
        !== canonicalJson(qpdfBudgetExec.binary)) {
    throw new Error("qpdf budget launcher receipt does not bind production binary");
  }
  await verifyQpdfMacosBudgetExecBuild(qpdfBudgetReceipt.value, {
    sourcePath: path.join(
      candidatePath,
      "test/eval/native/qpdf-macos-budget-exec.c",
    ),
    binaryPath: qpdfBudgetExecPath,
  ...
  await campaignV4Internals.verifyCorpus(
    corpusAuthority,
    candidate,
    runtime,
  );
  ...
  const plan = {
    ...
    qpdf,
    qpdf_budget_exec: qpdfBudgetExec,
    runtime,
    timeouts,
  ...
  process.stdout.write(`${canonicalJson({
    protocol: "pdf-tools.deep-malformed-macos-campaign-plan-prepared.v4",
    logical_run_label: logicalRunLabel,
