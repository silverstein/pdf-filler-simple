 ...
 * captured call wall clocks, supervisor samples/peaks, and raw response
 * identities containing distinct
 * planned argument paths or exact-path validated product volatility, and raw
 * output identities containing explicitly located clock metadata. Each
 * individual campaign still retains every raw response and output identity.
 */
...
const CAMPAIGN_PROTOCOL = "pdf-tools.deep-malformed-macos-campaign-result.v4";
const COMPARISON_PROTOCOL =
  "pdf-tools.deep-malformed-macos-campaign-reproducibility.v4";
...
function validReceiptRow(receipt, row, index) {
  if (!exactKeys(row, [
    ...
    || !campaignV4Internals.validateCampaignSupervisorEvidence(
    ...
    || row.candidate_stdout?.record?.protocol
      !== "pdf-tools.deep-malformed-row-result.v4"
  ...
  const rederivedAssertions = campaignV4Internals.validateRowRecord(
...
function receiptAuthoritiesValid(receipt, expectedLabel, planAuthority) {
  try {
    campaignV4Internals.validatePlan(receipt?.plan?.value);
  } catch {
    return false;
  }
...
function scannerSemantics(scanner) {
  return {
    ...
    response_normalization: scanner.response_normalization,
...
async function main() {
  if (process.platform !== "darwin"
    ...
    throw new Error(
      "Usage: compare-deep-malformed-macos-campaign-v4.js "
  ...
  await campaignV4Internals.verifyCandidate(authorityPlan.candidate);
  await campaignV4Internals.verifySupervisor(
    authorityPlan.supervisor,
    authorityPlan.candidate,
  );
  await campaignV4Internals.verifyQpdfBudgetExec(
  ...
  process.stdout.write(`${canonicalJson({
    protocol:
      "pdf-tools.deep-malformed-macos-campaign-reproducibility-written.v4",
    comparison: identity,
    summary: comparison.result,
  })}\n`);
}

export const campaignComparisonV4Internals = Object.freeze({
  inventorySemantics,
  logicalRow,
  normalizedPlan,
  outputSemantics,
