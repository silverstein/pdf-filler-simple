...
const CAMPAIGN_PROTOCOL = "pdf-tools.deep-malformed-macos-campaign-result.v2";
const COMPARISON_PROTOCOL =
  "pdf-tools.deep-malformed-macos-campaign-reproducibility.v2";
...
function validReceiptRow(receipt, row, index) {
  if (!exactKeys(row, [
    ...
    || !campaignV2Internals.validateCampaignSupervisorEvidence(
    ...
    || row.candidate_stdout?.record?.protocol
      !== "pdf-tools.deep-malformed-row-result.v2"
  ...
  const rederivedAssertions = campaignV2Internals.validateRowRecord(
...
function receiptAuthoritiesValid(receipt, expectedLabel, planAuthority) {
  try {
    campaignV2Internals.validatePlan(receipt?.plan?.value);
  } catch {
    return false;
  }
...
function outputSemantics(outputValidation) {
  return {
    ...
    outputs: outputValidation.outputs.map(output => ({
      ...
      qpdf_check_pass: output.qpdf_check.outcome === "close"
        && output.qpdf_check.code === 0
        && output.qpdf_check.signal === null
        && output.qpdf_check.timed_out === false
        && output.qpdf_check.output_overflow === false,
      qpdf_pages_pass: output.qpdf_pages.outcome === "close"
        && output.qpdf_pages.code === 0
        && output.qpdf_pages.signal === null
        && output.qpdf_pages.timed_out === false
        && output.qpdf_pages.output_overflow === false
        && output.qpdf_pages.stdout.text.trim() === "1",
      ...
      semantic_fingerprint: {
        ...
        canonicalization: {
          ...
          command_pass:
            ...
            && output.semantic_fingerprint.canonicalization.command.outcome
              === "close"
            && output.semantic_fingerprint.canonicalization.command.code === 0
            && output.semantic_fingerprint.canonicalization.command.signal
              === null
            && output.semantic_fingerprint.canonicalization.command.timed_out
              === false
            && output.semantic_fingerprint.canonicalization.command
...
async function main() {
  if (process.platform !== "darwin"
    ...
    throw new Error(
      "Usage: compare-deep-malformed-macos-campaign-v2.js "
  ...
  await campaignV2Internals.verifyCandidate(authorityPlan.candidate);
  await campaignV2Internals.verifySupervisor(
  ...
  process.stdout.write(`${canonicalJson({
    protocol:
      "pdf-tools.deep-malformed-macos-campaign-reproducibility-written.v2",
    comparison: identity,
    summary: comparison.result,
  })}\n`);
}

export const campaignComparisonV2Internals = Object.freeze({
  inventorySemantics,
  logicalRow,
  normalizedPlan,
  outputSemantics,
