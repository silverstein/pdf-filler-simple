...
/**
 * One exact, native-supervised product row for the qpdf-contained v4 campaign.
 *
...
const REQUEST_PROTOCOL = "pdf-tools.deep-malformed-row-request.v4";
const RESULT_PROTOCOL = "pdf-tools.deep-malformed-row-result.v4";
const SCANNER_PROTOCOL = "pdf-tools.deep-malformed-response-leak-scan.v4";
const RESPONSE_NORMALIZATION_PROTOCOL =
  "pdf-tools.deep-malformed-response-semantic-normalization.v1";
...
const CALL_CLOCK_SKEW_MS = 1_000;
...
function validateExecutableIdentity(value, maximumBytes = 4 << 20) {
  return exactKeys(value, ["bytes", "links", "mode", "path", "sha256"])
    && typeof value.path === "string"
    && path.resolve(value.path) === value.path
    && integer(value.bytes, 1, maximumBytes)
    && value.links === 1
    && [0o700, 0o755].includes(value.mode)
    && SHA256.test(value.sha256);
}

function validateQpdfBudgetExec(value) {
  return exactKeys(value, ["binary", "policy"])
    && validateExecutableIdentity(value.binary)
    && validQpdfPolicy(value.policy)
...
async function readRequest() {
  ...
  if (canonicalJson(request) !== text
    ...
    || !validateQpdfBudgetExec(request.qpdf_budget_exec)) {
...
function exactOwnPath(value, segments) {
  let cursor = value;
...
function validCallWallClock(value) {
  return value
    && typeof value === "object"
    && !Array.isArray(value)
    && exactKeys(value, ["end_ms", "start_ms"])
    && integer(value.start_ms, 0, Number.MAX_SAFE_INTEGER)
    && integer(value.end_ms, value.start_ms, Number.MAX_SAFE_INTEGER);
}

function responseVolatilityRules(value, toolName, callWallClock) {
  const successfulCallToolResult = value
    && typeof value === "object"
    && !Array.isArray(value)
    && Array.isArray(value.content)
    && value.isError !== true;
  if (!successfulCallToolResult
    || !["read_pdf_fields", "rotate_pdf_pages"].includes(toolName)) {
    return { equalityGroups: [], rules: [] };
  }
  if (!validCallWallClock(callWallClock)) {
    throw new Error(
      "Approved response volatility requires a captured call wall clock",
    );
  }

  const definitions = toolName === "read_pdf_fields"
    ? [{
        segments: ["_meta", "viewUUID"],
        terminalKey: "viewUUID",
        valueClass: "pdf-view-uuid:epoch-millis:base36-6:near-call",
        epochMillis: candidate => {
          if (typeof candidate !== "string") return null;
          const match = /^pdf-([1-9]\d{12})-([a-z0-9]{6})$/.exec(candidate);
          return match === null ? null : Number(match[1]);
        },
      }]
    : [
        {
          segments: ["structuredContent", "last_mutation_at"],
          terminalKey: "last_mutation_at",
          valueClass: "iso-8601:utc-millisecond:near-call",
          epochMillis: candidate =>
            canonicalIsoTimestamp(candidate) ? Date.parse(candidate) : null,
        },
        {
          segments: ["_meta", "last_mutation_at"],
          terminalKey: "last_mutation_at",
          valueClass: "iso-8601:utc-millisecond:near-call",
          epochMillis: candidate =>
            canonicalIsoTimestamp(candidate) ? Date.parse(candidate) : null,
  ...
  const rules = definitions.map(definition => {
    const observed = exactOwnPath(value, definition.segments);
    const epochMillis = observed.present
      ? definition.epochMillis(observed.value)
      : null;
    if (!Number.isSafeInteger(epochMillis)) {
      throw new Error(
        `Approved volatile response field is absent or invalid: ${
          jsonPointer(definition.segments)
        }`,
      );
    }
    if (epochMillis < callWallClock.start_ms - CALL_CLOCK_SKEW_MS
      || epochMillis > callWallClock.end_ms + CALL_CLOCK_SKEW_MS) {
      throw new Error(
        `Approved volatile response field is not fresh: ${
          jsonPointer(definition.segments)
        }`,
      );
    }
    return {
      epochMillis,
      path: jsonPointer(definition.segments),
      segments: definition.segments,
      terminalKey: definition.terminalKey,
  ...
  if (toolName === "rotate_pdf_pages") {
    const paths = rules.map(rule => rule.path).sort();
    const values = rules.map(rule => rule.value);
    if (new Set(values).size !== 1) {
      throw new Error(
        "Approved rotate response timestamps must be byte-identical",
      );
    }
    equalityGroups.push({
      equal: true,
      paths,
      value_class: "iso-8601:utc-millisecond:near-call",
    });
  }
  return { equalityGroups, rules };
}

function approvedVolatileNormalizedString(rule) {
  return {
    protocol: NORMALIZED_STRING_PROTOCOL,
    segments: [{
      kind: "approved_volatile",
      path: rule.path,
      value_class: rule.valueClass,
    }],
  };
}
...
function scanStringLeaves(
  ...
  const volatility = responseVolatilityRules(
    value,
    toolName,
    callWallClock,
  );
  const volatilityByPath = new Map(
    volatility.rules.map(rule => [rule.path, rule]),
  );
  const retainedNonvolatileOccurrences = new Map(
    [...new Set(volatility.rules.map(rule => rule.terminalKey))]
      .map(key => [key, 0]),
  );
  ...
  const observeString = (
    current,
    { permitPathSubstitution, volatilityRule = null },
  ) => {
    scannedStrings += 1;
    ...
    const normalized = volatilityRule
      ? approvedVolatileNormalizedString(volatilityRule)
      : permitPathSubstitution
        ...
        : current;
    const normalizedBytes = permitPathSubstitution || volatilityRule
    ...
    if (volatilityRule) {
      if (RAW_INTERNAL_PATTERN.test(current)) rawInternalMatches += 1;
    } else if (permitPathSubstitution) {
  ...
  while (stack.length > 0) {
    ...
    if (typeof current === "string") {
      const pointer = jsonPointer(semanticPath);
      parent[key] = observeString(current, {
        permitPathSubstitution: true,
        volatilityRule: volatilityByPath.get(pointer) ?? null,
      });
      ...
      for (let index = current.length - 1; index >= 0; index -= 1) {
        stack.push({
          ...
          parent: normalized,
          key: index,
          semanticPath: [...semanticPath, String(index)],
        });
      }
      ...
      for (const childKey of keys) {
        const childSemanticPath = [...semanticPath, childKey];
        const childPointer = jsonPointer(childSemanticPath);
        if (retainedNonvolatileOccurrences.has(childKey)
          && !volatilityByPath.has(childPointer)) {
          retainedNonvolatileOccurrences.set(
            childKey,
            retainedNonvolatileOccurrences.get(childKey) + 1,
          );
        }
        scannedNodes += 1;
        if (scannedNodes > limits.response_nodes) {
        ...
        stack.push({
          ...
          parent: normalized,
          key: childKey,
          semanticPath: childSemanticPath,
        });
      }
    } else {
      parent[key] = current;
  ...
  return {
    ...
    response_normalization: {
      protocol: RESPONSE_NORMALIZATION_PROTOCOL,
      tool_name: toolName,
      excluded_fields: volatility.rules.map(rule => rule.path),
      excluded_occurrences: Object.fromEntries(
        volatility.rules.map(rule => [rule.path, 1]),
      ),
      excluded_value_classes: Object.fromEntries(
        volatility.rules.map(rule => [rule.path, rule.valueClass]),
      ),
      equality_groups: volatility.equalityGroups,
      freshness_policy: {
        allowed_skew_ms: CALL_CLOCK_SKEW_MS,
        basis: "captured-call-wall-clock",
        checked_fields: volatility.rules.map(rule => rule.path),
      },
      retained_nonvolatile_occurrences: Object.fromEntries(
        [...retainedNonvolatileOccurrences.entries()]
...
function responseSummary(
  ...
  const standardStructuredErrorSummary = structuredError
  ...
  const contentFailureSummary = toolName === "read_pdf_content"
    && structuredError
    && typeof structuredError === "object"
    && !Array.isArray(structuredError)
    && exactKeys(structuredError, contentFailureKeys)
    && typeof structuredError.pdf_path === "string"
    && typeof structuredError.file_name === "string"
    && Number.isInteger(structuredError.total_pages)
    && Number.isInteger(structuredError.pages_read)
    && Number.isInteger(structuredError.text_length)
    && structuredError.text_truncated === false
    && structuredError.text_found === false
    && structuredError.content_available === false
    && structuredError.extraction_status === "failed"
    && Array.isArray(structuredError.page_previews)
    && structuredError.preview_truncated === false
    && structuredError.extraction_mode === "none"
    && Array.isArray(structuredError.error_codes)
    && structuredError.error_codes.length > 0
    && structuredError.error_codes.every(value => typeof value === "string")
    && typeof structuredError.retry_guidance === "string"
    ? {
        status: "failed",
    ...
    : null;
  const structuredErrorSummary =
    standardStructuredErrorSummary ?? contentFailureSummary;
  const scanner = scanStringLeaves(
    response,
    allowedPaths,
    limits,
    toolName,
  ...
  try {
    ...
    return {
      ...
      response: responseSummary(
        ...
        limits,
        name,
        { end_ms: callEndMs, start_ms: callStartMs },
      ),
    };
  } catch (error) {
    return {
...
function inheritedContainmentIdentity() {
  const result = spawnSync(
    "/bin/ps",
    ["-o", "pid=,pgid=", "-p", String(process.pid)],
    {
      encoding: "utf8",
      env: { LANG: "C", LC_ALL: "C", PATH: "/usr/bin:/bin" },
      maxBuffer: 64 * 1024,
  ...
  if (result.error || result.status !== 0 || result.signal !== null
      || result.stderr !== "") {
    throw new Error("Unable to capture inherited qpdf containment identity");
  }
  const fields = result.stdout.trim().split(/\s+/).map(Number);
  if (fields.length !== 2 || fields[0] !== process.pid
      || !integer(fields[1], 2, 2147483647)) {
    throw new Error("Inherited qpdf containment identity is malformed");
  }
  return Object.freeze({ pid: fields[0], pgid: fields[1] });
}
...
function qpdfInputMatchesFileIdentity(actual, expected) {
...
async function runContainedQpdf(request, containmentIdentity, inputPath, args, {
  expectedInputIdentity,
  inputMaxBytes,
  timeoutMs,
  outputMaxBytes,
}) {
  const { version: _version, ...qpdfIdentity } = request.qpdf;
  const input = await openQpdfBudgetInput(
    inputPath,
    inputMaxBytes,
    [0o400, 0o600],
  );
  try {
    if (!qpdfInputMatchesFileIdentity(
      input.identity,
      expectedInputIdentity,
    )) {
      throw new Error("QPDF input differs from its prevalidated identity");
    }
    return await runBudgetedQpdfCommand({
      launcher: request.qpdf_budget_exec.binary,
      qpdf: qpdfIdentity,
      input,
      args,
      expectedPgid: containmentIdentity.pgid,
      timeoutMs,
      outputMaxBytes,
      policy: request.qpdf_budget_exec.policy,
    });
  } finally {
...
function budgetedQpdfPassed(observation) {
  return observation?.protocol === QPDF_BUDGET_COMMAND_PROTOCOL
    && observation.pass === true
    && observation.containment?.budget_enforced === true
    && observation.containment?.control_eof_after_ready === true
    && observation.containment?.same_process_group === true
    && observation.containment?.same_session === true
    && observation.containment?.executables_stable === true;
}
...
async function semanticFingerprintPdf(
  ...
  try {
    ...
    const projected = await runContainedQpdf(
      request,
      containmentIdentity,
      outputPath,
      [
      ...
      {
        expectedInputIdentity: outputIdentity,
        inputMaxBytes: canonicalPdfMaxBytes,
        timeoutMs,
        outputMaxBytes,
      },
    );
    const projectedCommand = boundedQpdfCommandEvidence(projected);
    const projectedCommandPass = budgetedQpdfPassed(projected)
      && projected.command.stdout.bytes === 0
      && projected.command.stderr.bytes === 0;
    ...
    const observed = await runContainedQpdf(
      request,
      containmentIdentity,
      canonicalPath,
      ["--json-output=2", "/dev/fd/4"],
      {
        expectedInputIdentity: projectedIdentity,
        inputMaxBytes: canonicalPdfMaxBytes,
        timeoutMs,
        outputMaxBytes,
      },
    );
    command = boundedQpdfCommandEvidence(observed);
    const commandPass = budgetedQpdfPassed(observed)
      && observed.command.stderr.bytes === 0;
    if (!commandPass) {
      return {
        protocol: OUTPUT_FINGERPRINT_PROTOCOL,
        canonicalization,
    ...
    const value = JSON.parse(observed.command.stdout.text);
    ...
    return {
      ...
      pass: qpdfInputMatchesFileIdentity(
        projected.containment.input,
        outputIdentity,
      ) && qpdfInputMatchesFileIdentity(
        observed.containment.input,
        projectedIdentity,
      ),
    };
  } catch (error) {
...
async function validateSuccessfulOutput({
  ...
  const checked = await runContainedQpdf(
    request,
    containmentIdentity,
    outputPath,
    ["--check", "/dev/fd/4"],
    {
      expectedInputIdentity: outputIdentity,
      inputMaxBytes: request.evidence_limits.per_file_bytes,
      timeoutMs: request.evidence_limits.qpdf_timeout_ms,
      outputMaxBytes: request.evidence_limits.qpdf_output_bytes,
    },
  );
  const qpdfCheckPass = budgetedQpdfPassed(checked);
  const pages = qpdfCheckPass
    ? await runContainedQpdf(
        request,
        containmentIdentity,
        outputPath,
        ["--show-npages", "/dev/fd/4"],
        {
          expectedInputIdentity: outputIdentity,
          inputMaxBytes: request.evidence_limits.per_file_bytes,
          timeoutMs: request.evidence_limits.qpdf_timeout_ms,
    ...
    : null;
  const semanticFingerprint = qpdfCheckPass
    ? await semanticFingerprintPdf(
        request,
        containmentIdentity,
        outputPath,
        outputIdentity,
    ...
    : {
        ...
        normalized: null,
        pass: false,
        skipped_after_failed_check: true,
      };
  ...
  const qpdfPagesPass = budgetedQpdfPassed(pages)
    && pages.command.stdout.text.trim() === "1";
  ...
  return {
    ...
    pass: nonalias
      ...
      && qpdfPagesPass
      && qpdfInputMatchesFileIdentity(
        checked.containment.input,
        outputIdentity,
      )
      && qpdfInputMatchesFileIdentity(
        pages.containment.input,
        outputIdentity,
      )
      && semanticPass
    ...
    outputs: [{
      ...
      qpdf_check: boundedQpdfCommandEvidence(checked, { retainStdoutText: true }),
      qpdf_pages: pages
        ? boundedQpdfCommandEvidence(pages, { retainStdoutText: true })
        : null,
      semantic,
...
async function main() {
  ...
  const containmentIdentity = inheritedContainmentIdentity();
  ...
  const result = {
    ...
    execution: {
      ...
      inherited_containment: containmentIdentity,
...
export const rowRunnerV4Internals = Object.freeze({
...
if (process.argv[1]
  ...
  main().catch(error => {
    const failure = {
      protocol: "pdf-tools.deep-malformed-row-harness-error.v4",
