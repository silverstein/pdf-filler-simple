...
/**
 * One exact, native-supervised product row for the qpdf-contained v3 campaign.
 *
...
const REQUEST_PROTOCOL = "pdf-tools.deep-malformed-row-request.v3";
const RESULT_PROTOCOL = "pdf-tools.deep-malformed-row-result.v3";
const SCANNER_PROTOCOL = "pdf-tools.deep-malformed-response-leak-scan.v3";
...
function responseSummary(response, allowedPaths, limits, toolName = null) {
...
async function callTool(client, name, args, timeout, allowedPaths, limits) {
  try {
    ...
    return {
      outcome: "response",
      response: responseSummary(response, allowedPaths, limits, name),
    };
  } catch (error) {
    return {
...
export const rowRunnerV3Internals = Object.freeze({
...
if (process.argv[1]
  ...
  main().catch(error => {
    const failure = {
      protocol: "pdf-tools.deep-malformed-row-harness-error.v3",
