...
function validateStoredEvent(event, identity, policy, expectedSequence) {
  ...
  assertion(event.contract?.name === "pdf-tools.verified-extraction-workspace-event"
    && event.contract?.version === "1.0.0-experimental", "event contract is unsupported");
  assertion(event.kind === "proposal_submitted", "event kind is unsupported");
...
function deriveState(identity, events, generationSequence) {
  ...
  return {
    ...
    pending_leaf_count: identity.leaf_obligations.length,
    result_count: 0,
    proposals,
    results: [],
  };
...
async function readWorkspaceStatic(workspacePath, expectedWorkspaceIdentitySha256 = null) {
  ...
  assertion(identity.package_inclusion === "disabled_experimental", "workspace package boundary drifted");
...
async function scanWorkspace(workspacePath, staticContext, { allowClaim = false } = {}) {
  ...
  for (const entry of workspaceEntries) {
    const isStaticFile = [IDENTITY_FILE, MAP_FILE, LEAVES_FILE, POINTER_FILE].includes(entry.name);
  ...
  assertion([IDENTITY_FILE, MAP_FILE, LEAVES_FILE, POINTER_FILE, "generations"]
...
export async function appendUnverifiedWorkspaceProposal({
  ...
  const eventBody = {
    contract: { name: "pdf-tools.verified-extraction-workspace-event", version: "1.0.0-experimental" },
...
function generationCollections(staticContext, generation) {
  return {
    ...
    pending_leaves: staticContext.identity.leaf_obligations,
...
async function assertPreGenesisInitializationScratch(directory) {
  await assertNoLinksRecursively(directory);
  const allowed = new Set([IDENTITY_FILE, MAP_FILE, LEAVES_FILE, POINTER_FILE, "generations"]);
  const entries = await readdir(directory, { withFileTypes: true });
  for (const entry of entries) {
